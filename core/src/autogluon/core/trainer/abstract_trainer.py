...
class AbstractTrainer:
    ...
    def _train_single(self, X, y, model: AbstractModel, X_val=None, y_val=None, total_resources=None, is_pretrain=False, **model_fit_kwargs) -> AbstractModel:
        """
        Trains model but does not add the trained model to this Trainer.
        Returns trained model object.
        """
        model = model.fit(X=X, y=y, X_val=X_val, y_val=y_val, total_resources=total_resources, is_pretrain=is_pretrain, **model_fit_kwargs)
        return model
        ...
        **model_fit_kwargs
        ...
        try:
            ...
            if not isinstance(model, BaggedEnsembleModel) and X_pseudo is not None and y_pseudo is not None and X_pseudo.columns.equals(X.columns):
                ...
                model = self._train_single(X, y, model, X_val, y_val, total_resources=total_resources, is_pretrain=is_pretrain, **model_fit_kwargs)
